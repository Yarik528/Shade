// language: Java, file: src/main/java/com/shade/gui/ClickGuiScreen.java
...
public class ClickGuiScreen extends Screen {
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        for (Module m : filteredModules()) {
            ...
            if (expanded == m) {
                modY = renderSettings(matrices, m, modX, modY, modW);
            }
        }
    ...
    private int renderSettings(MatrixStack matrices, Module m, int x, int y, int w) {
        if (m instanceof KillAura) {
            ...
            y = slider(matrices, "Min CPS", x + 6, y + 2, w - 12, 1f, 20f, (float) k.getMinCps(), v -> k.setMinCps(v));
            y = slider(matrices, "Max CPS", x + 6, y + 2, w - 12, 1f, 20f, (float) k.getMaxCps(), v -> k.setMaxCps(v));
            y = slider(matrices, "FOV", x + 6, y + 2, w - 12, 30f, 360f, (float) k.getFov(), v -> k.setFov(v));
        } else if (m instanceof Reach) {
            Reach r = (Reach) m;
            y = slider(matrices, "Block Reach", x + 6, y + 2, w - 12, 3f, 6f, (float) r.getBlockReachRaw(), v -> r.setBlockReach(v));
            y = slider(matrices, "Entity Reach", x + 6, y + 2, w - 12, 3f, 6f, (float) r.getEntityReachRaw(), v -> r.setEntityReach(v));
        } else if (m instanceof SilentAim) {
            SilentAim s = (SilentAim) m;
            y = toggleRow(matrices, "GCD Snap", x + 6, y + 2, w - 12, s.isGcdSnap());
            y = toggleRow(matrices, "Jitter", x + 6, y + 2, w - 12, s.isMicroJitter());
        } else if (m instanceof Velocity) {
            Velocity v = (Velocity) m;
            y = slider(matrices, "Horizontal", x + 6, y + 2, w - 12, 0f, 1f, (float) v.getHorizontalRaw(), val -> v.setHorizontal(val));
            y = slider(matrices, "Vertical", x + 6, y + 2, w - 12, 0f, 1f, (float) v.getVerticalRaw(), val -> v.setVertical(val));
            y = toggleRow(matrices, "AntiDetect", x + 6, y + 2, w - 12, v.isAntiDetect());
        }
        return y + 6;
    }

    private int slider(MatrixStack matrices, String label, int x, int y, int w,
                       float min, float max, float val, Consumer<Float> onChange) {
    ...
    private int toggleRow(MatrixStack matrices, String label, int x, int y, int w,
        ...
        textRenderer.draw(matrices, label, x, y, TEXT_DIM);
        int bx = x + w - 22;
        fill(matrices, bx, y, bx + 20, y + 9, on ? ACCENT : BG_MODULE);
        fill(matrices, on ? bx + 11 : bx + 1, y + 1, on ? bx + 19 : bx + 9, y + 8, TEXT_ENABLED);
        return y + 14;
    }
    ...
    private String categoryOf(Module m) {
        String n = m.getName();
        switch (n) {
            case "KillAura": case "SilentAim": case "Reach": case "Velocity":
                return "Combat";
            case "ESP": case "XRay":
                return "Render";
            default:
    ...
    public boolean mouseClicked(double mx, double my, int button) {
        ...
        for (Module m : filteredModules()) {
            ...
            if (expanded == m) {
                int h = settingsHeight(m);
                if (mx >= modX && mx <= modX + modW
                 && my >= modY && my <= modY + h) {
                    if (tryClickSettings(m, modX, modY, modW, mx, my)) return true;
                }
                modY += h;
            }
        }
    ...
    private int settingsHeight(Module m) {
        if (m instanceof KillAura) return 3 * 21 + 6;
        if (m instanceof Reach) return 2 * 21 + 6;
        if (m instanceof SilentAim) return 2 * 14 + 6;
        if (m instanceof Velocity) return 2 * 21 + 14 + 6;
        return 0;
    }

    private boolean tryClickSettings(Module m, int x, int y, int w, double mx, double my) {
        if (m instanceof KillAura) {
            KillAura k = (KillAura) m;
            if (hitSlider(x + 6, y + 13, w - 12, mx, my)) {
                dragging = makeSlider(x + 6, y + 13, w - 12, 1f, 20f, (float) k.getMinCps(), v -> k.setMinCps(v));
                applyDrag(mx); return true;
            }
            if (hitSlider(x + 6, y + 34, w - 12, mx, my)) {
                dragging = makeSlider(x + 6, y + 34, w - 12, 1f, 20f, (float) k.getMaxCps(), v -> k.setMaxCps(v));
                applyDrag(mx); return true;
            }
            if (hitSlider(x + 6, y + 55, w - 12, mx, my)) {
                dragging = makeSlider(x + 6, y + 55, w - 12, 30f, 360f, (float) k.getFov(), v -> k.setFov(v));
                applyDrag(mx); return true;
            }
        } else if (m instanceof Reach) {
            Reach r = (Reach) m;
            ...
            if (hitSlider(x + 6, y + 34, w - 12, mx, my)) {
                dragging = makeSlider(x + 6, y + 34, w - 12, 3f, 6f, (float) r.getEntityReachRaw(), v -> r.setEntityReach(v));
                applyDrag(mx); return true;
            }
        } else if (m instanceof Velocity) {
            Velocity v = (Velocity) m;
            ...
            if (hitSlider(x + 6, y + 34, w - 12, mx, my)) {
                dragging = makeSlider(x + 6, y + 34, w - 12, 0f, 1f, (float) v.getVerticalRaw(), val -> v.setVertical(val));
                applyDrag(mx); return true;
            }
        } else if (m instanceof SilentAim) {
            SilentAim s = (SilentAim) m;
            if (mx >= x + w - 28 && mx <= x + w - 8 && my >= y + 2 && my <= y + 11) {
                s.setGcdSnap(!s.isGcdSnap());
                ConfigManager.save(); return true;
            }
            if (mx >= x + w - 28 && mx <= x + w - 8 && my >= y + 16 && my <= y + 25) {
    ...
    private boolean hitSlider(int x, int y, int w, double mx, double my) {
        return mx >= x && mx <= x + w && my >= y - 4 && my <= y + 8;
    }
